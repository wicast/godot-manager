...
public partial class GifAnimatedTexture   {
	...
	public GifAnimatedTexture(string file) {
		...
		for(int i = 0; i < Texture.Frames; i++) {
			using(MemoryStream ms = new MemoryStream()) {
				...
				Godot.ImageTexture gditFrame = Godot.ImageTexture.CreateFromImage(img);
				gdFrames.Add(gditFrame);
#pragma warning disable CS0618
				Texture.SetFrameTexture(i, gditFrame);
				Texture.SetFrameDuration(i, gifMeta.FrameDelay * 0.01f);
#pragma warning restore CS0618
			}
		}

		Godot.ResourceSaver.Save(Texture, file + ".res", Godot.ResourceSaver.SaverFlags.BundleResources & Godot.ResourceSaver.SaverFlags.Compress);
	}
}
